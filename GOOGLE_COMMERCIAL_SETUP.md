...
Dado que la plataforma es multi-tenant, **NO usaremos Service Accounts por cliente**. Cada cliente autenticará su propia cuenta de Google mediante OAuth 2.0. El dueño de la plataforma Sodare (tú) debe registrar UNA aplicación en Google Cloud Platform (GCP).

## 1. Crear el Proyecto en Google Cloud
1. Entra a [Google Cloud Console](https://console.cloud.google.com).
2. Crea un nuevo proyecto llamado **Sodare Hub Comercial**.
...
5. **Authorized domains**: Agrega tu dominio de producción (ej. `sodare.xyz`).
...
3. **Name**: Sodare Web.
4. **Authorized JavaScript origins**: `https://sodare.xyz` (y `http://localhost:3000` para dev).
5. **Authorized redirect URIs**: `https://sodare.xyz/api/oauth/google/callback` (y `http://localhost:3000/api/oauth/google/callback` para dev).
6. Copia el **Client ID** y el **Client Secret**.

## 5. Configurar Variables de Entorno en Sodare
...
2. Sigue los pasos de verificación. Google te pedirá:
   - Un video de YouTube mostrando cómo Sodare usa los datos.

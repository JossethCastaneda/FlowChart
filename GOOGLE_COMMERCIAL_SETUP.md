...
Dado que la plataforma es multi-tenant, **NO usaremos Service Accounts por cliente**. Cada cliente autenticará su propia cuenta de Google mediante OAuth 2.0. El dueño de la plataforma Zefirus (tú) debe registrar UNA aplicación en Google Cloud Platform (GCP).

## 1. Crear el Proyecto en Google Cloud
1. Entra a [Google Cloud Console](https://console.cloud.google.com).
2. Crea un nuevo proyecto llamado **Zefirus Hub Comercial**.
...
5. **Authorized domains**: Agrega tu dominio de producción (ej. `zefirus.xyz`).
...
3. **Name**: Zefirus Web.
4. **Authorized JavaScript origins**: `https://zefirus.xyz` (y `http://localhost:3000` para dev).
5. **Authorized redirect URIs**: `https://zefirus.xyz/api/oauth/google/callback` (y `http://localhost:3000/api/oauth/google/callback` para dev).
...
2. Sigue los pasos de verificación. Google te pedirá:
   - Un video de YouTube mostrando cómo Zefirus usa los datos.

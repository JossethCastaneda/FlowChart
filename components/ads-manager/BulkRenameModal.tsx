        ...
        <div style={{ padding: "16px 20px" }}>
            ...
            {([
              ...
              <button
                ...
                style={{
                  ...
                  color: mode === t.key ? "var(--cyan)" : "rgba(148,163,184,0.5)",
                }}
              >
                {t.label}
              </button>
          ...
          {mode === "prefix" && (
            <div style={{ marginBottom: "14px" }}>
              <label style={{ fontSize: "10px", color: "rgba(148,163,184,0.5)", fontWeight: 600, marginBottom: "4px", display: "block" }}>Prefijo</label>
              <input value={prefix} onChange={(e) => setPrefix(e.target.value)} placeholder="Ej: [Q1]_" style={inputStyle} autoFocus />
            </div>
          )}
          {mode === "suffix" && (
            <div style={{ marginBottom: "14px" }}>
              <label style={{ fontSize: "10px", color: "rgba(148,163,184,0.5)", fontWeight: 600, marginBottom: "4px", display: "block" }}>Sufijo</label>
          ...
          {mode === "replace" && (
            <div style={{ marginBottom: "14px" }}>
              <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: "10px", color: "rgba(148,163,184,0.5)", fontWeight: 600, marginBottom: "4px", display: "block" }}>Buscar</label>
                  <input value={searchText} onChange={(e) => setSearchText(e.target.value)} placeholder="Texto a buscar" style={inputStyle} autoFocus />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: "10px", color: "rgba(148,163,184,0.5)", fontWeight: 600, marginBottom: "4px", display: "block" }}>Reemplazar con</label>
                  <input value={replaceText} onChange={(e) => setReplaceText(e.target.value)} placeholder="Texto nuevo" style={inputStyle} />
                </div>
              </div>
              <div style={{ display: "flex", gap: "12px" }}>
                <label style={{ display: "flex", alignItems: "center", gap: "4px", fontSize: "10px", color: "rgba(148,163,184,0.5)", cursor: "pointer" }}>
          ...
          <div style={{ maxHeight: "200px", overflowY: "auto", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.04)" }} className="custom-scrollbar">
            {preview.map((p) => (
              <div key={p.id} style={{
                ...
                <span style={{ fontSize: "10px", color: "rgba(148,163,184,0.6)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {p.oldName}
                </span>
                <ArrowRight className="w-3 h-3" style={{ color: p.changed ? "var(--cyan)" : "rgba(148,163,184,0.2)", flexShrink: 0 }} />
                <span style={{ fontSize: "10px", color: p.changed ? "white" : "rgba(148,163,184,0.3)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: p.changed ? 600 : 400 }}>
                  {p.newName}
                </span>
              </div>
            ))}
        ...
        <div style={{ display: "flex", gap: "8px", padding: "12px 20px", borderTop: "1px solid var(--border)", justifyContent: "flex-end" }}>
          ...
          <button
            ...
            style={{
              ...
              border: `1px solid ${affectedCount > 0 ? "rgba(0,212,255,0.25)" : "rgba(148,163,184,0.1)"}`,
              color: affectedCount > 0 ? "var(--cyan)" : "rgba(148,163,184,0.3)",

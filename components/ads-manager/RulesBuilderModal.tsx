  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: "560px", maxWidth: "90vw", background: "var(--fc-surface)", 
        ...
        <div style={{ padding: "16px 20px", minHeight: "280px", maxHeight: "400px", overflowY: "auto" }} className="custom-scrollbar">
          {step === 1 && (
            <>
              <div style={{ marginBottom: "14px" }}>
                <label style={{ fontSize: "10px", color: "var(--fc-text-muted)", fontWeight: 600, display: "block", marginBottom: "4px" }}>Nombre de la regla</label>
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ej: Pausar CPC alto" style={inputStyle} autoFocus />
              </div>
              <div style={{ marginBottom: "14px" }}>
                <label style={{ fontSize: "10px", color: "var(--fc-text-muted)", fontWeight: 600, display: "block", marginBottom: "4px" }}>Aplicar a</label>
              ...
              <div style={{ marginBottom: "14px" }}>
                <label style={{ fontSize: "10px", color: "var(--fc-text-muted)", fontWeight: 600, display: "block", marginBottom: "4px" }}>Frecuencia de evaluación</label>
          ...
          {step === 2 && (
            <>
              <div style={{ marginBottom: "14px" }}>
                <label style={{ fontSize: "10px", color: "var(--fc-text-muted)", fontWeight: 600, display: "block", marginBottom: "4px" }}>Ventana de tiempo (aplica a todas las condiciones)</label>
              ...
              {conditions.map((cond, idx) => (
                <div key={idx} style={{ display: "flex", gap: "6px", marginBottom: "10px", alignItems: "flex-end" }}>
                  <div style={{ flex: 2 }}>
                    <label style={{ fontSize: "9px", color: "var(--fc-text-muted)", display: "block", marginBottom: "2px" }}>Métrica</label>
                    <select value={cond.field} onChange={(e) => updateCondition(idx, "field", e.target.value)} style={{ ...selectStyle, fontSize: "11px" }}>
                      {METRICS.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}
                    </select>
                  </div>
                  <div style={{ flex: 1.5 }}>
                    <label style={{ fontSize: "9px", color: "var(--fc-text-muted)", display: "block", marginBottom: "2px" }}>Operador</label>
                  ...
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: "9px", color: "var(--fc-text-muted)", display: "block", marginBottom: "2px" }}>Valor</label>
                  ...
                  <button onClick={() => removeCondition(idx)} disabled={conditions.length <= 1} style={{ background: "none", border: "none", color: conditions.length > 1 ? "var(--fc-danger)" : "rgba(148,163,184,0.65)", cursor: "pointer", padding: "8px", flexShrink: 0 }}>
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
              ))}
              <button onClick={addCondition} style={{ display: "flex", alignItems: "center", gap: "4px", fontSize: "11px", fontWeight: 600, color: "var(--fc-accent)", background: "none", border: "none", cursor: "pointer", padding: "4px 0" }}>
          ...
          {step === 3 && (
            <>
              <div style={{ marginBottom: "14px" }}>
                <label style={{ fontSize: "10px", color: "var(--fc-text-muted)", fontWeight: 600, display: "block", marginBottom: "4px" }}>Acción a ejecutar</label>
              ...
              {action === "CHANGE_BUDGET" && (
                <div style={{ marginBottom: "14px" }}>
                  <label style={{ fontSize: "10px", color: "var(--fc-text-muted)", fontWeight: 600, display: "block", marginBottom: "4px" }}>Ajuste de presupuesto (%)</label>
                  <input value={budgetAdjustment} onChange={(e) => setBudgetAdjustment(e.target.value)} placeholder="+10 o -20" style={inputStyle} />
                  <span style={{ fontSize: "9px", color: "var(--fc-text-muted)", marginTop: "4px", display: "block" }}>
          ...
          {step === 4 && (
            <>
              <div style={{ background: "var(--fc-accent-wash)", border: "1px solid rgba(59,130,246,0.08)", borderRadius: "8px", padding: "14px" }}>
                <div style={{ fontSize: "11px", color: "var(--fc-text)", fontWeight: 700, marginBottom: "10px" }}>Resumen de la regla</div>
                <div style={{ fontSize: "11px", color: "var(--fc-text-secondary)", lineHeight: "1.8" }}>
                  <div><strong style={{ color: "var(--fc-text)" }}>Nombre:</strong> {name || "—"}</div>
                  <div><strong style={{ color: "var(--fc-text)" }}>Aplica a:</strong> {entityType === "CAMPAIGN" ? "Campañas" : entityType === "ADSET" ? "Conjuntos" : "Anuncios"}</div>
                  <div><strong style={{ color: "var(--fc-text)" }}>Frecuencia:</strong> {FREQUENCIES.find((f) => f.value === frequency)?.label}</div>
                  <div><strong style={{ color: "var(--fc-text)" }}>Condiciones:</strong></div>
                  {conditions.map((c, i) => (
                    <div key={i} style={{ paddingLeft: "12px", fontSize: "10px" }}>
                      {i > 0 && <span style={{ color: "var(--fc-accent)", fontWeight: 600 }}>AND </span>}
                  ...
                  <div><strong style={{ color: "var(--fc-text)" }}>Acción:</strong> {ACTIONS.find((a) => a.value === action)?.label}</div>
        ...
        {/* Error */}
        {error && (
          <div style={{ display: "flex", alignItems: "center", gap: "6px", padding: "0 20px 10px", fontSize: "11px", color: "var(--fc-danger)" }}>
            <AlertCircle className="w-3.5 h-3.5" /> {error}
          </div>
        )}

        {/* Footer */}
        <div style={{ display: "flex", gap: "8px", padding: "12px 20px", borderTop: "1px solid var(--fc-border)", justifyContent: "space-between" }}>
          <button
            onClick={() => step > 1 ? setStep(step - 1) : onClose()}
            style={{ display: "flex", alignItems: "center", gap: "4px", padding: "7px 14px", fontSize: "11px", fontWeight: 600, background: "var(--surface-hover)", border: "1px solid var(--hairline)", borderRadius: "6px", color: "var(--fc-text-secondary)", cursor: "pointer" }}
          >
          ...
          {step < 4 ? (
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: "4px", padding: "7px 14px", fontSize: "11px", fontWeight: 600, background: "var(--fc-accent-wash)", border: "1px solid rgba(59,130,246,0.25)", borderRadius: "6px", color: "var(--fc-accent)", cursor: "pointer" }}
            >
            ...
            <button
              ...
              disabled={loading}
              style={{ padding: "7px 14px", fontSize: "11px", fontWeight: 600, background: "var(--fc-accent-wash)", border: "1px solid rgba(59,130,246,0.25)", borderRadius: "6px", color: "var(--fc-accent)", cursor: "pointer", opacity: loading ? 0.5 : 1 }}
            >

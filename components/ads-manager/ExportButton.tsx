...
export function ExportButton({ data, level, visibleColumns }: ExportButtonProps) {
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `flowchart_${level}_${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
...
<Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>
    ...
    import("jszip").then((JSZipModule) => {
      ...
      zip.generateAsync({ type: "blob", mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }).then((blob) => {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = `flowchart_${level}_${new Date().toISOString().split("T")[0]}.xlsx`;
        a.click();
        URL.revokeObjectURL(url);
      });
    }).catch(() => {
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              padding: "8px 10px", fontSize: "11px", color: "var(--fc-text)",
          ...
          >
            <Sheet className="w-3.5 h-3.5" style={{ color: "var(--fc-success)" }} /> Exportar Excel
          </button>
          <button
            onClick={exportCSV}
            style={{
            ...
            onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
          >
            <Table2 className="w-3.5 h-3.5" style={{ color: "var(--fc-success)" }} /> Exportar CSV
          </button>
          <button
            ...
            onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
          >
            <FileText className="w-3.5 h-3.5" style={{ color: "var(--fc-module-aria)" }} /> Imprimir / PDF
          </button>
        </div>
      )}
    </div>

...
export function AdsExecutiveSummary({
  ...
  const cards = [
    { label: "Gasto", value: formatMoney(spend), icon: DollarSign, color: "#06d6a0" },
    { label: "ROAS promedio", value: avgRoas ? `${avgRoas.toFixed(2)}x` : "Sin datos", icon: TrendingUp, color: "#00d4ff" },
    { label: "Clics", value: formatCompact(clicks), icon: MousePointerClick, color: "#ffbe0b" },
    { label: "Impresiones", value: formatCompact(impressions), icon: RefreshCw, color: "#7b61ff" },
  ];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      >
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, color: "#64748b", fontSize: 11 }}>
      ...
      {error && (
        <div style={{ display: "flex", gap: 10, padding: 12, borderRadius: 8, border: "1px solid rgba(255,45,85,0.25)", background: "rgba(255,45,85,0.08)", color: "#fb7185", fontSize: 12 }}>
          ...
          return (
            <div key={card.label} className="kpi-card" style={{ padding: 18 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                <div>
                  <div style={{ fontSize: 12, color: "#94a3b8", fontWeight: 600 }}>{card.label}</div>
                  <div style={{ marginTop: 8, fontSize: 24, color: "#f8fafc", fontWeight: 800 }}>{card.value}</div>
                </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.1fr) minmax(0, 0.9fr)", gap: 12 }}>
        <div className="glass-panel" style={{ padding: 18 }}>
          <h3 style={{ margin: 0, color: "#f8fafc", fontSize: 14, fontWeight: 700 }}>
            Riesgos detectados
          </h3>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 }}>
            {warnings.length === 0 ? (
              <div style={{ display: "flex", gap: 9, color: "#22c55e", fontSize: 12, alignItems: "center" }}>
                <CheckCircle2 style={{ width: 15, height: 15 }} />
                No hay alertas obvias en los datos cargados.
              </div>
            ) : (
              warnings.map((warning) => (
                <div key={warning.title} style={{ display: "flex", gap: 10, padding: 10, borderRadius: 8, background: "rgba(255,255,255,0.035)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  <AlertTriangle
                    style={{
                      width: 16,
                      height: 16,
                      flexShrink: 0,
                      color: warning.level === "high" ? "#fb7185" : warning.level === "medium" ? "#fbbf24" : "#60a5fa",
                    }}
                  />
                  <div>
                    <div style={{ fontSize: 12, color: "#f8fafc", fontWeight: 700 }}>{warning.title}</div>
                    <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{warning.description}</div>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="glass-panel" style={{ padding: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 12 }}>
            {[
              ...
              <div key={label} style={{ padding: 10, borderRadius: 8, background: "rgba(255,255,255,0.035)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div style={{ fontSize: 10, color: "#64748b", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>{label}</div>
                <div style={{ marginTop: 5, fontSize: 20, color: "#e2e8f0", fontWeight: 800 }}>{value}</div>
              </div>
            ))}
          </div>
        </div>

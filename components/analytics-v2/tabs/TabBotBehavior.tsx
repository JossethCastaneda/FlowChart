...
function Kpi({ label, value, sub, color = "var(--cyan)", icon }: { label: string; value: string; sub?: string; color?: string; icon?: React.ReactNode }) {
  return (
...
export function TabBotBehavior({ query, base }: { query: string; base: string }) {
  ...
  if (!data) return <div style={{ padding: 60, textAlign: "center", color: "var(--text-muted)" }}>Sin datos.</div>;

  if (!data.connected) {
    return (
      <div style={{ ...panel, padding: 48, textAlign: "center" }}>
        <Bot className="w-10 h-10 mx-auto mb-3" style={{ color: "var(--text-muted)" }} />
        <p style={{ color: "var(--text-secondary)", fontSize: 14 }}>La plataforma analítica del proyecto no está conectada.</p>
        <p style={{ color: "var(--text-muted)", fontSize: 12, marginTop: 6 }}>Asocia Botmaker o Cari AI y sincroniza para ver el análisis.</p>
      </div>
    );
  }
  ...
  return <div style={{ padding: 60, textAlign: "center", color: "var(--text-muted)" }}>Sin resultados para este periodo.</div>;
}
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 150, fontSize: 12, color: "var(--foreground)", textAlign: "right" }}>{i + 1}. {s.label}</div>
            <div style={{ flex: 1, background: "rgba(255,255,255,0.05)", borderRadius: 6, height: 26, position: "relative" }}>
              ...
              <span style={{ position: "absolute", left: 8, top: 4, fontSize: 11, color: "var(--foreground)" }}>{s.reached.toLocaleString("es-MX")}</span>
            </div>
            <div style={{ width: 90, fontSize: 11, color: s.dropOff ? "var(--red)" : "var(--text-muted)" }}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12 }}>
        <Kpi label="1ª respuesta (prom.)" value={fmtDuration(b.responseTimes.avgFirstResponseSec)} icon={<Clock className="w-3.5 h-3.5" />} color="var(--cyan)" />
        <Kpi label="Respuesta del bot (prom.)" value={fmtDuration(b.responseTimes.avgBotSec)} icon={<Clock className="w-3.5 h-3.5" />} color="var(--emerald)" />
        <Kpi label="Respuesta del usuario (prom.)" value={fmtDuration(b.responseTimes.avgUserSec)} icon={<Clock className="w-3.5 h-3.5" />} color="var(--purple)" />
        <Kpi label="Conversaciones analizadas" value={b.sampleSize.toLocaleString("es-MX")} icon={<MessageSquare className="w-3.5 h-3.5" />} color="var(--amber)" />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: 16 }}>
          ...
          {msgData.length ? (
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={msgData} layout="vertical" margin={{ left: 12, right: 16 }}>
                ...
                <YAxis type="category" dataKey="name" width={92} tick={{ fill: "var(--text-secondary)", fontSize: 11 }} />
                <Tooltip cursor={{ fill: "var(--surface-hover)" }} contentStyle={{ background: "var(--foreground)", border: "1px solid var(--hairline)", borderRadius: 8, fontSize: 12 }} formatter={(value: unknown, _name: unknown, entry: unknown) => {
        ...
        <div style={panel}>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
            ...
            <Mini label="Tasa de selección" value={`${Math.round(b.buttons.selectRate * 100)}%`} color="var(--emerald)" />
          </div>
          {b.buttons.topButtons.length ? (
            <div style={{ maxHeight: 200, overflowY: "auto" }}>
              <table style={{ width: "100%", fontSize: 12, color: "var(--foreground)" }}>
                <thead><tr style={{ color: "var(--text-muted)", textAlign: "left" }}><th style={{ padding: "4px 6px" }}>Botón</th><th>Mostrado</th><th>Elegido</th><th>CTR</th></tr></thead>
                <tbody>
                  {b.buttons.topButtons.map((bt) => (
                    <tr key={bt.label} style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                      ...
                      <td style={{ color: bt.ctr >= 50 ? "var(--emerald)" : bt.ctr > 0 ? "var(--amber)" : "var(--text-muted)" }}>{bt.ctr}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* Errores del bot */}
        <div style={panel}>
          <h3 style={h3}><AlertTriangle className="w-4 h-4" style={{ color: "var(--red)" }} /> Errores del bot</h3>
          <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
            <Mini label="Errores totales" value={b.errors.total.toLocaleString("es-MX")} color="var(--red)" />
          ...
          {b.errors.byType.length ? (
            <div style={{ maxHeight: 180, overflowY: "auto" }}>
              {b.errors.byType.map((e) => (
                <div key={e.type} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--foreground)", padding: "4px 0", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                  <span style={{ maxWidth: 240, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.type}</span>
                  <span style={{ color: "var(--red)" }}>{e.count}</span>
                </div>
              ))}
            </div>
        ...
        <div style={panel}>
          <h3 style={h3}><ShoppingCart className="w-4 h-4" style={{ color: "var(--emerald)" }} /> Ventas (felicidades) y tiempo a cierre</h3>
          <div style={{ display: "flex", gap: 16, marginBottom: 12, flexWrap: "wrap" }}>
            <Mini label="Ventas" value={b.timeToSale.count.toLocaleString("es-MX")} color="var(--emerald)" />
            <Mini label="Conversión" value={`${Math.round(b.timeToSale.conversionRate * 1000) / 10}%`} color="var(--emerald)" />
          ...
          {b.timeToSale.count ? (
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={ttsData} margin={{ left: 0, right: 8 }}>
                ...
                <Tooltip cursor={{ fill: "var(--surface-hover)" }} contentStyle={{ background: "var(--foreground)", border: "1px solid var(--hairline)", borderRadius: 8, fontSize: 12 }} />
        ...
        {/* NIP */}
        <div style={panel}>
          <h3 style={h3}><KeyRound className="w-4 h-4" style={{ color: "var(--amber)" }} /> NIP · obtención y tiempo</h3>
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            <Mini label="Bot pidió NIP" value={b.nip.prompted.toLocaleString("es-MX")} />
            <Mini label="Respondió (cualquiera)" value={b.nip.responded.toLocaleString("es-MX")} color="var(--purple)" />
            <Mini label="Entrega válida" value={b.nip.delivered.toLocaleString("es-MX")} color="var(--emerald)" />
            <Mini label="Tasa de entrega" value={`${Math.round(b.nip.firstResponseRate * 1000) / 10}%`} />
          </div>
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginTop: 10 }}>
            <Mini label="Tiempo 1ª respuesta" value={fmtDuration(b.nip.avgRespondedSec)} color="var(--purple)" />
              ...
              {b.firstMenu.byType.map((r, i) => {
                ...
                return (
                  <div key={r.type} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 120, fontSize: 12, color: "var(--foreground)", textAlign: "right" }}>{r.label}</div>
                    <div style={{ flex: 1, background: "rgba(255,255,255,0.05)", borderRadius: 6, height: 24, position: "relative" }}>
                      ...
                      <span style={{ position: "absolute", left: 8, top: 3, fontSize: 11, color: "var(--foreground)" }}>{r.count.toLocaleString("es-MX")} · {r.pct}%</span>
                    </div>
                  </div>
                );
              })}
        ...
        {b.objections.length ? (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {b.objections.map((o, i) => (
              <span key={o.label} style={{ background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.2)", color: "var(--foreground)", padding: "6px 10px", borderRadius: 8, fontSize: 12 }}>
      ...
      <div style={panel}>
        <h3 style={h3}><AlertTriangle className="w-4 h-4" style={{ color: "var(--red)" }} /> Rechazos de portabilidad (detectados en conversación)</h3>
        <div style={{ color: "var(--text-muted)", fontSize: 11, marginBottom: 10 }}>
          Sesiones con mensaje de rechazo del bot: <strong style={{ color: "var(--red)" }}>{b.rejections.total.toLocaleString("es-MX")}</strong>. El conteo definitivo de &quot;primer rechazo Botmaker&quot; (ventas dashboard − ventas en sábana) requiere el cruce con la sábana de ventas.
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
          {b.rejections.byReason.map((r) => (
            <div key={r.key} style={{ background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.25)", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "var(--foreground)" }}>
              {r.label}: <strong style={{ color: "var(--red)" }}>{r.count.toLocaleString("es-MX")}</strong>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
        <div style={panel}>
          <h3 style={h3}><GitBranch className="w-4 h-4" style={{ color: "var(--purple)" }} /> Derivaciones a agente</h3>
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            <Mini label="Sesiones derivadas" value={b.derivations.count.toLocaleString("es-MX")} color="var(--purple)" />
            <Mini label="Tasa de derivación" value={`${Math.round(b.derivations.rate * 1000) / 10}%`} />
          </div>
          <div style={{ color: "var(--text-muted)", fontSize: 10, marginTop: 8 }}>Transferencias a un agente humano (mensaje de agente o evento de transferencia).</div>
        </div>
        <div style={panel}>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            ...
            <Mini label="eSIM" value={b.simEsim.esim.toLocaleString("es-MX")} color="var(--emerald)" />
            <Mini label="Sin dato" value={b.simEsim.sinDato.toLocaleString("es-MX")} />
          </div>
          <div style={{ color: "var(--text-muted)", fontSize: 10, marginTop: 8 }}>Por menciones en la conversación. Relevante en Lira.</div>
        </div>
        <div style={panel}>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            ...
            <Mini label="Con respuesta" value={b.reactivations.reactivated.toLocaleString("es-MX")} color="var(--emerald)" />
            <Mini label="Tasa de éxito" value={`${Math.round(b.reactivations.rate * 1000) / 10}%`} />
          </div>
          <div style={{ color: "var(--text-muted)", fontSize: 10, marginTop: 8 }}>Mensaje del bot tras ≥30 min de silencio que logró respuesta.</div>
        </div>
      </div>
    </div>
  );
...
function CariBehavior({ cari }: { cari: CariResults }) {
  return (
    <div className="space-y-6">
      <div style={{ ...panel, padding: 14, color: "var(--text-secondary)", fontSize: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12 }}>
        <Kpi label="Conversaciones" value={cari.kpis.totalConversations.toLocaleString("es-MX")} color="var(--cyan)" />
        <Kpi label="Contención del bot" value={`${cari.kpis.botContainmentPct}%`} color="var(--emerald)" />
        <Kpi label="Transferidas" value={cari.kpis.transferred.toLocaleString("es-MX")} color="var(--amber)" />
        <Kpi label="Abandonadas" value={`${cari.kpis.abandonedPct}%`} color="var(--red)" />
        <Kpi label="Interacciones (prom.)" value={String(cari.kpis.avgInteractions)} color="var(--purple)" />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: 16 }}>
          ...
          {cari.funnel.length ? (
            <div className="space-y-2">
              {cari.funnel.map((s, i) => {
                ...
                return (
                  <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 160, fontSize: 12, color: "var(--foreground)", textAlign: "right" }}>{s.label}</div>
                    <div style={{ flex: 1, background: "rgba(255,255,255,0.05)", borderRadius: 6, height: 26, position: "relative" }}>
                      <div style={{ width: `${w}%`, background: PALETTE[i % PALETTE.length], height: "100%", borderRadius: 6 }} />
                      <span style={{ position: "absolute", left: 8, top: 4, fontSize: 11, color: "var(--foreground)" }}>{s.count.toLocaleString("es-MX")}</span>
                    </div>
                  </div>
                );
              })}
        ...
        <div style={panel}>
          <h3 style={h3}><AlertTriangle className="w-4 h-4" style={{ color: "var(--amber)" }} /> Razones de no-finalización</h3>
          {cari.dropOffReasons.length ? (
            <div className="space-y-2">
              {cari.dropOffReasons.map((r) => (
                <div key={r.key} style={{ fontSize: 12, color: "var(--foreground)", padding: "6px 0", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}><span>{r.label}</span><span style={{ color: "var(--amber)" }}>{r.count} ({r.pct}%)</span></div>
                </div>
              ))}
            </div>
          ) : <Empty />}
      ...
      <div style={panel}>
        <h3 style={h3}><MessageSquare className="w-4 h-4" style={{ color: "var(--red)" }} /> Frases sin respuesta del bot ({cari.botErrors.totalUnanswered})</h3>
        {cari.botErrors.unanswered.length ? (
          <div style={{ maxHeight: 220, overflowY: "auto" }}>
            {cari.botErrors.unanswered.slice(0, 20).map((u, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--foreground)", padding: "4px 0", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                <span style={{ maxWidth: 360, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.phrase}</span>
                <span style={{ color: "var(--red)" }}>{u.count}×</span>
              </div>
            ))}
          </div>
      ...
      {cari.insights.length > 0 && (
        <div style={panel}>
          ...
          <ul style={{ margin: 0, paddingLeft: 18, color: "var(--foreground)", fontSize: 12, lineHeight: 1.7 }}>
  ...
  return (
    <div style={{ ...panel, borderColor: "rgba(0,212,255,0.25)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 12, marginBottom: 14 }}>
        <Mini label="Sesiones" value={b.sampleSize.toLocaleString("es-MX")} color="var(--cyan)" />
        <Mini label="Ventas (felicidades)" value={b.timeToSale.count.toLocaleString("es-MX")} color="var(--emerald)" />
        <Mini label="Conversión" value={`${conv}%`} color="var(--emerald)" />
        <Mini label="NIP entregado" value={`${nipRate}%`} color="var(--amber)" />
        <Mini label="Rechazos detectados" value={b.rejections.total.toLocaleString("es-MX")} color="var(--red)" />
        <Mini label="Errores del bot" value={b.errors.total.toLocaleString("es-MX")} color="var(--red)" />
      </div>
      {findings.length > 0 && (
        <div>
          <div style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 6 }}>Hallazgos accionables</div>
          <ul style={{ margin: 0, paddingLeft: 18, color: "var(--foreground)", fontSize: 12, lineHeight: 1.7 }}>
            {findings.map((f, i) => <li key={i}>{f}</li>)}
          </ul>
        </div>
      )}
      <div style={{ color: "var(--text-muted)", fontSize: 10, marginTop: 10 }}>Fuente: conversaciones Botmaker en vivo (regla de venta = mensaje del bot con &quot;felicidades&quot;).</div>
    </div>
  );
}
...
function SalesReconciliation({ base, query }: { base: string; query: string }) {
  ...
  const chip: React.CSSProperties = { background: "var(--surface-hover)", border: "1px solid var(--hairline)", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "var(--foreground)" };

  return (
    <div style={panel}>
      <h3 style={h3}><ShoppingCart className="w-4 h-4" style={{ color: "var(--emerald)" }} /> Cruce con sábana de ventas (éxitos y rechazos)</h3>
      <div style={{ color: "var(--text-muted)", fontSize: 11, marginBottom: 10 }}>
      ...
      <label style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(0,212,255,0.1)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", borderRadius: 8, padding: "8px 14px", fontSize: 12, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer" }}>
        {loading ? "Procesando…" : "Subir sábana (CSV/XLSX)"}
        <input type="file" accept=".csv,.xlsx,.xls" style={{ display: "none" }} disabled={loading}
          onChange={(e) => { const f = e.target.files?.[0]; if (f) onFile(f); }} />
      </label>
      {fileName && <span style={{ marginLeft: 10, fontSize: 11, color: "var(--text-secondary)" }}>{fileName}</span>}
      {error && <div style={{ color: "var(--red)", fontSize: 12, marginTop: 10 }}>{error}</div>}
      {result && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 12 }}>
            <Mini label="Ventas dashboard (felicidades)" value={result.dashboardSales.toLocaleString("es-MX")} color="var(--cyan)" />
            <Mini label="Exitosas (en sábana)" value={result.exitosas.toLocaleString("es-MX")} color="var(--emerald)" />
            <Mini label="Primer rechazo Botmaker" value={result.firstRejection.toLocaleString("es-MX")} color="var(--red)" />
          ...
          {result.firstRejection > 0 && (
            <div style={{ background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 8, padding: 12 }}>
              <div style={{ fontSize: 12, color: "var(--foreground)", marginBottom: 6 }}>El primer rechazo Botmaker puede corresponder a estos 2 motivos (sin reparto artificial):</div>
              <ul style={{ margin: 0, paddingLeft: 18, color: "var(--foreground)", fontSize: 12, lineHeight: 1.6 }}>
          ...
          {result.byCapturista.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 6 }}>Exitosas por capturista</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {result.byCapturista.map((c) => <span key={c.key} style={chip}>{c.key}: <strong style={{ color: "var(--emerald)" }}>{c.count}</strong></span>)}
              </div>
            </div>
          )}
          {result.byBot.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 6 }}>Exitosas por bot</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {result.byBot.map((c) => <span key={c.key} style={chip}>{c.key}: <strong style={{ color: "var(--cyan)" }}>{c.count}</strong></span>)}
              </div>
            </div>
          )}
        </div>
...
function ByBotSection({ byBot }: { byBot: BotSummary[] }) {
  ...
  const th: React.CSSProperties = { padding: "6px 8px", color: "var(--text-muted)", fontWeight: 600, textAlign: "left", whiteSpace: "nowrap" };
  const td: React.CSSProperties = { padding: "6px 8px", color: "var(--foreground)", whiteSpace: "nowrap" };
                  ...
                  <tr onClick={() => setExpanded(open ? null : bot.channelId)}
                    ...
                    <td style={{ ...td, maxWidth: 220 }}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, maxWidth: "100%", overflow: "hidden" }}>
                        <ChevronDown className="w-3 h-3" style={{ transform: open ? "rotate(0)" : "rotate(-90deg)", transition: "transform .15s", color: "var(--text-muted)", flexShrink: 0 }} />
                    ...
                    <td style={{ ...tdR, color: "var(--emerald)" }}>{bot.sales.toLocaleString("es-MX")}</td>
                    ...
                    <tr>
                      <td colSpan={8} style={{ padding: "12px 8px", background: "rgba(255,255,255,0.02)" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 16 }}>
                          <div>
                            <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 8 }}>Funnel 1 · reacción al primer menú</div>
                            {bot.firstMenu.total ? (
                              <div className="space-y-2">
                                {bot.firstMenu.byType.map((r, i) => {
                                  ...
                                  return (
                                    <div key={r.type} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                      <div style={{ width: 110, fontSize: 11, color: "var(--foreground)", textAlign: "right" }}>{r.label}</div>
                                      <div style={{ flex: 1, background: "rgba(255,255,255,0.05)", borderRadius: 6, height: 20, position: "relative" }}>
                                        <div style={{ width: `${w}%`, background: PALETTE[i % PALETTE.length], height: "100%", borderRadius: 6 }} />
                                        <span style={{ position: "absolute", left: 6, top: 2, fontSize: 10, color: "var(--foreground)" }}>{r.count.toLocaleString("es-MX")} · {r.pct}%</span>
                                      </div>
                                    </div>
                                  );
                                })}
                              </div>
                            ) : <div style={{ color: "var(--text-muted)", fontSize: 11 }}>Sin datos.</div>}
                          </div>
                          <div>
                            <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 8 }}>
                              Funnel 2 · {bot.flowType ? `tipo: ${bot.flowType}` : "orden inferido"}
                            </div>
                            {bot.funnel2.steps.length ? <FunnelSteps steps={bot.funnel2.steps} /> : <div style={{ color: "var(--text-muted)", fontSize: 11 }}>Sin orden detectado.</div>}
                          </div>
                        </div>
                      </td>
                    </tr>
...
function Mini({ label, value, color = "var(--foreground)" }: { label: string; value: string; color?: string }) {
  return (
    <div>
...
function Empty({ text = "Sin datos para este periodo." }: { text?: string }) {
  return <div style={{ color: "var(--text-muted)", fontSize: 12, padding: "24px 0", textAlign: "center" }}>{text}</div>;
}

    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel" style={{ padding: "24px" }}>
          <h3 style={{ color: "var(--foreground)", fontSize: "16px", fontWeight: 600, display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
            <Database className="w-5 h-5 text-cyan-400" /> Sincronización y Origen
          </h3>
          <div className="space-y-4">
            <div>
              <p style={{ fontSize: "12px", color: "var(--text-secondary)", marginBottom: "4px" }}>Modo de Datos</p>
              <div style={{ display: "inline-block", background: "rgba(255,255,255,0.05)", padding: "4px 12px", borderRadius: "12px", fontSize: "12px", color: "var(--foreground)" }}>
                {scope.allowedProviders?.length ? "Real (Conectado)" : "Mock (Pruebas)"}
              </div>
            </div>
            <div>
              <p style={{ fontSize: "12px", color: "var(--text-secondary)", marginBottom: "4px" }}>Canales Activos</p>
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {scope.allowedChannels?.map(c => (
                  <span key={c} style={{ background: "rgba(0,212,255,0.1)", color: "var(--cyan)", padding: "2px 8px", borderRadius: "4px", fontSize: "11px" }}>{c}</span>
                )) || <span style={{ color: "var(--text-muted)", fontSize: "12px" }}>Todos</span>}
              </div>
            </div>
            ...
            <div style={{ marginTop: "24px", paddingTop: "16px", borderTop: "1px solid var(--hairline)" }}>
              <button 
                ...
                disabled={syncing}
                style={{ background: "var(--cyan)", border: "none", color: "var(--foreground)", padding: "8px 16px", borderRadius: "8px", fontSize: "12px", fontWeight: 700, display: "flex", alignItems: "center", gap: "6px", cursor: syncing ? "not-allowed" : "pointer" }}
              >
                <RefreshCw className={`w-4 h-4 ${syncing ? "animate-spin" : ""}`} /> 
                {syncing ? "Sincronizando..." : "Sincronizar Manualmente"}
              </button>
              {successMsg && <p style={{ fontSize: "12px", color: "var(--emerald)", marginTop: "8px" }}>{successMsg}</p>}
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: "24px" }}>
          ...
          <div className="space-y-3 max-h-[360px] overflow-y-auto pr-2 custom-scrollbar">
            {targets.length === 0 && <p style={{ color: "var(--text-muted)", fontSize: "12px" }}>Cargando metas...</p>}
            {targets.map((t) => {
              ...
              const inputStyle: React.CSSProperties = { width: "100%", background: "rgba(0,0,0,0.3)", border: "1px solid var(--hairline)", color: "var(--foreground)", fontSize: 12, borderRadius: 6, padding: "4px 8px", outline: "none" };
              return (
                <div key={t.kpiKey} style={{ background: "var(--row-hover)", padding: "12px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.05)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                    <span style={{ fontSize: "13px", fontWeight: 500, color: "var(--foreground)" }} title={t.description || ""}>{t.name}</span>
                    <span style={{ fontSize: "11px", color: t.overridden ? "var(--cyan)" : "var(--text-muted)" }}>{t.overridden ? "Override proyecto" : "Global"}</span>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: "10px", alignItems: "end" }}>
                    <label>
                      <span style={{ fontSize: "10px", color: "var(--emerald)", display: "block", marginBottom: 4 }}>Meta (Verde){unit ? ` ${unit}` : ""}</span>
                    ...
                    <label>
                      <span style={{ fontSize: "10px", color: "var(--red)", display: "block", marginBottom: 4 }}>Advertencia (Rojo){unit ? ` ${unit}` : ""}</span>
                      <input type="number" step="any" style={inputStyle}
                    ...
                    <button
                      ...
                      style={{ background: dirty ? "var(--cyan)" : "rgba(255,255,255,0.05)", color: dirty ? "var(--foreground)" : "var(--text-muted)", border: "none", borderRadius: 6, padding: "6px 12px", fontSize: 11, fontWeight: 700, cursor: dirty ? "pointer" : "not-allowed", whiteSpace: "nowrap" }}>
        ...
        <div className="glass-panel md:col-span-2" style={{ padding: "24px" }}>
          ...
          <p style={{ fontSize: "12px", color: "var(--text-secondary)", maxWidth: "600px" }}>

...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { color: string; icon: React.ReactNode; label: string }> = {
    completed: { color: "var(--emerald)", icon: <CheckCircle2 className="w-3.5 h-3.5" />, label: "Completado" },
    failed: { color: "var(--red)", icon: <XCircle className="w-3.5 h-3.5" />, label: "Fallido" },
    running: { color: "var(--amber)", icon: <Clock className="w-3.5 h-3.5" />, label: "En curso" },
    pending: { color: "var(--text-secondary)", icon: <Clock className="w-3.5 h-3.5" />, label: "Pendiente" },
  };
  const s = map[status] || { color: "var(--text-secondary)", icon: null, label: status };
  return (
...
export function ProjectAnalyticsConfigPanel({ projectId }: { projectId: string }) {
  ...
  if (isLoading) return <div className="glass-panel" style={{ padding: 32, textAlign: "center", color: "var(--text-muted)" }}>Cargando configuración…</div>;
  if (error) return <div className="glass-panel" style={{ padding: 32, textAlign: "center", color: "var(--red)" }}>{error instanceof Error ? error.message : "Error"}</div>;
  if (!data) return null;

  const card: React.CSSProperties = { padding: 20 };
  const label: React.CSSProperties = { fontSize: 11, letterSpacing: "0.15em", color: "var(--text-secondary)", textTransform: "uppercase", marginBottom: 10 };
  const chip: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 10px", borderRadius: 999, background: "rgba(148,163,184,0.12)", fontSize: 12, color: "var(--foreground)", marginRight: 8, marginBottom: 8 };
    ...
    <div className="space-y-4">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
        <div className="glass-panel" style={card}>
          ...
          {data.channels.length === 0 ? (
            <p style={{ color: "var(--text-muted)", fontSize: 12 }}>Sin canales configurados.</p>
          ) : (
        ...
        <div className="glass-panel" style={card}>
          ...
          {data.integrations.length === 0 ? (
            <p style={{ color: "var(--text-muted)", fontSize: 12 }}>Sin integraciones de analítica vinculadas.</p>
          ) : (
            data.integrations.map((i) => (
              <span key={i.id} style={chip}>
                <PlugZap className="w-3.5 h-3.5" style={{ color: i.connected ? "var(--emerald)" : "var(--text-secondary)" }} />
        ...
        <div className="glass-panel" style={card}>
          ...
          <p style={{ fontSize: 12, color: data.openAlerts > 0 ? "var(--amber)" : "var(--emerald)", marginTop: 6, display: "inline-flex", alignItems: "center", gap: 6 }}>
      ...
      <div className="glass-panel" style={card}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--text-muted)" }}>Trae los últimos 7 días de los proveedores configurados (idempotente).</p>
          </div>
          <button
        ...
        {syncMsg && <p style={{ fontSize: 12, color: "var(--foreground)", marginTop: 12 }}>{syncMsg}</p>}
      </div>
      ...
      <div className="glass-panel" style={card}>
        ...
        {data.recentJobs.length === 0 ? (
          <p style={{ color: "var(--text-muted)", fontSize: 12 }}>Aún no hay sincronizaciones registradas.</p>
        ) : (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {data.recentJobs.map((j) => (
                  <tr key={j.id} style={{ borderTop: "1px solid rgba(148,163,184,0.12)", color: "var(--foreground)" }}>
                    ...
                    <td style={{ padding: "6px 8px", color: "var(--red)", maxWidth: 220, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{j.errorMessage || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>

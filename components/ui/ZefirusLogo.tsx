...
interface ZefirusLogoProps {
  ...
  showText?: boolean;
  /** Muestra el subtítulo "SOFTWARE PLATFORM" */
...
/**
 * ZefirusLogo — Vectorizado pixel-perfecto del logo oficial.
 *
 * Usa los paths SVG exactos del diseño maestro (viewBox 0 0 1024 1024):
 * - Emblema: 3 paths del símbolo Z+flecha+grafo, gradiente cyan→azul
 * - Wordmark "ZEFIRUS": paths tipográficos exactos (#1de4f2)
 * - Subtitle "SOFTWARE PLATFORM": paths tipográficos exactos (#1db5e2)
 * - Animaciones: entrada fadeUp + glowPulse + sweep destello + node pulse
 */
export function ZefirusLogo({
  ...
  // Dimensiones del contenedor según size
  const widths = { xs: 80, sm: 140, md: 200, lg: 280, xl: 400 };
  const w = widths[size];

  return (
    <div
      className={className}
      style={{ display: "inline-block", width: w, ...style }}
    >
      {animated && (
        <style>{`
          @keyframes zfr-emblemIn {
            from { opacity:0; transform:scale(.92) translateY(18px); }
            60%  { opacity:1; }
            to   { opacity:1; transform:none; }
          }
          @keyframes zfr-glowPulse {
            0%,100% { filter:drop-shadow(0 0 7px rgba(38,230,248,.35)); }
            50%     { filter:drop-shadow(0 0 16px rgba(38,230,248,.6)); }
          }
          @keyframes zfr-fadeUp {
            from { opacity:0; transform:translateY(22px); }
            to   { opacity:1; transform:none; }
          }
          @keyframes zfr-nodeIn {
            from { opacity:0; transform:scale(.3); }
            to   { opacity:.55; transform:scale(1); }
          }
          @keyframes zfr-npulse {
          ...
          .zfr-emblem-${uid} {
            ...
            animation:
              zfr-emblemIn  1.1s .15s cubic-bezier(.2,.8,.25,1) forwards,
              zfr-glowPulse 4.5s 1.6s ease-in-out infinite;
          }
          .zfr-wordmark-${uid} {
            opacity:0;
            animation:zfr-fadeUp .85s .9s cubic-bezier(.2,.8,.25,1) forwards;
            filter:drop-shadow(0 0 8px rgba(30,220,240,.28));
          }
          .zfr-subtitle-${uid} {
            opacity:0;
            animation:zfr-fadeUp .85s 1.25s cubic-bezier(.2,.8,.25,1) forwards;
          }
          ...
          .zfr-nglow-${uid}.g0 { animation:zfr-nodeIn .4s 1.5s  forwards, zfr-npulse 3s 2.2s ease-in-out infinite; }
          .zfr-nglow-${uid}.g1 { animation:zfr-nodeIn .4s 1.65s forwards, zfr-npulse 3s 2.5s ease-in-out infinite; }
          .zfr-nglow-${uid}.g2 { animation:zfr-nodeIn .4s 1.8s  forwards, zfr-npulse 3s 2.8s ease-in-out infinite; }
          .zfr-nglow-${uid}.g3 { animation:zfr-nodeIn .4s 1.95s forwards, zfr-npulse 3s 3.1s ease-in-out infinite; }

          .zfr-shine-${uid} {
            transform:translateX(-620px);
            animation:zfr-sweep 5s 2.2s ease-in-out infinite;
          }

          @media (prefers-reduced-motion:reduce) {
            .zfr-emblem-${uid},
            .zfr-wordmark-${uid},
            .zfr-subtitle-${uid} { opacity:1!important; transform:none!important; animation:none!important; }
            .zfr-nglow-${uid} { opacity:.5!important; animation:none!important; }
      ...
      <svg
        ...
        role="img"
        aria-label="Zefirus — Software Platform"
        style={{ display: "block", overflow: "visible" }}
      >
        <defs>
          {/* Gradiente del emblema: cyan → azul (muestreado del original) */}
          <linearGradient id={`${uid}-gEmb`} x1="0" y1="250" x2="120" y2="590" gradientUnits="userSpaceOnUse">
          ...
          {/* Gradiente del destello que barre el emblema */}
          <linearGradient id={`${uid}-gShine`} x1="0" y1="0" x2="1" y2="0">
            <stop offset="0"  stopColor="#fff" stopOpacity="0" />
            <stop offset=".5" stopColor="#fff" stopOpacity=".55" />
            <stop offset="1"  stopColor="#fff" stopOpacity="0" />
          </linearGradient>

          {/* Filtro blur para halos de nodos */}
          <filter id={`${uid}-soft`} x="-60%" y="-60%" width="220%" height="220%">
            <feGaussianBlur stdDeviation="9" />
          </filter>

          {/* ─── EMBLEMA (Z + flecha + grafo de red) ─── */}
          <g id={`${uid}-embPaths`} transform="translate(0,1024) scale(0.1,-0.1)">
            <path d="M6850 7659 c-223 -59 -451 -120 -507 -135 -57 -14 -103 -30 -103 -35
...
50 c90 107 196 177 313 206 38 9 185 13 550 13 l499 1 -122 -135z"/>
            <path d="M6990 6438 c-25 -5 -55 -10 -68 -12 -32 -5 -92 -70 -92 -100 0 -14
...
0 6 4 12 9 12 5 0 11 13 14 28 6 32 -18 27 352 71 189 23 187 23 180 11z"/>
            <path d="M5314 5566 c-70 -31 -318 -83 -479 -101 -117 -13 -331 -56 -430 -86
...
120 76 225 116 85 33 110 39 110 28z"/>
          ...
          {/* ─── WORDMARK "ZEFIRUS" ─── */}
          <g id={`${uid}-wordPaths`} transform="translate(0,1024) scale(0.1,-0.1)">
            <path d="M7593 3885 c-77 -21 -125 -49 -163 -91 -46 -53 -60 -93 -60 -171 0
...
4 0 19 29 35 65 33 80 31 84 -77 122 -93 32 -255 41 -336 18z"/>
            <path d="M2210 3815 l0 -75 245 0 c208 0 245 -2 238 -14 -9 -15 -203 -252
...
0 60 0 60 -365 0 -365 0 0 -75z"/>
            <path d="M3160 3440 l0 -450 338 2 337 3 3 73 3 72 -256 0 -255 0 0 120 0 120
220 0 220 0 0 70 0 70 -220 0 -220 0 0 110 0 110 245 0 245 0 0 75 0 75 -330
0 -330 0 0 -450z"/>
            <path d="M4065 3878 c-3 -7 -4 -209 -3 -448 l3 -435 85 0 85 0 3 173 2 172
221 0 220 0 -3 73 -3 72 -217 3 -218 2 0 125 0 125 245 0 245 0 0 75 0 75
-330 0 c-255 0 -332 -3 -335 -12z"/>
            <path d="M4935 3878 c-3 -7 -4 -209 -3 -448 l3 -435 85 0 85 0 3 448 2 447
-85 0 c-59 0 -87 -4 -90 -12z"/>
            <path d="M5382 3443 l3 -448 85 0 85 0 3 133 3 132 112 -1 112 0 90 -132 90
...
c116 -3 143 -7 177 -25z"/>
            <path d="M6357 3883 c-11 -11 -8 -535 4 -598 15 -84 46 -142 104 -201 77 -77
...
-86 -3 -89 -7z"/>
          ...
          {/* ─── SUBTÍTULO "SOFTWARE PLATFORM" ─── */}
          <g id={`${uid}-subPaths`} transform="translate(0,1024) scale(0.1,-0.1)">
            <path d="M2685 2718 c-31 -17 -45 -39 -45 -73 0 -40 21 -59 90 -82 42 -14 55
...
-72 61 11 13 24 15 71 10 54 -7 58 -6 58 14 0 35 -97 55 -145 28z"/>
            <path d="M2972 2711 c-64 -41 -86 -108 -58 -174 23 -54 59 -80 119 -85 96 -8
...
168 25 14 47 15 80 2z"/>
            <path d="M3265 2717 c-3 -7 -4 -69 -3 -138 l3 -124 28 -3 c26 -3 27 -2 27 47
l0 50 63 3 c55 3 62 5 65 25 3 20 -1 22 -60 25 l-63 3 -3 37 -3 37 73 3 c65 3
73 5 73 23 0 18 -8 20 -98 23 -74 2 -99 0 -102 -11z"/>
            <path d="M3510 2705 c0 -23 4 -25 40 -25 l40 0 0 -115 0 -115 30 0 29 0 3 113
3 112 43 3 c38 3 43 6 40 25 -3 21 -8 22 -115 25 l-113 3 0 -26z"/>
            <path d="M3780 2708 c1 -13 18 -75 38 -138 37 -111 39 -115 69 -118 30 -3 31
...
-94 -45 -86 -2 3 -17 41 -32 85 -23 64 -32 80 -50 82 -18 3 -23 -1 -23 -20z"/>
            <path d="M4317 2703 c-29 -55 -107 -233 -107 -243 0 -6 11 -10 25 -10 17 0 29
...
-70z"/>
            <path d="M4556 2714 c-8 -20 -8 -125 0 -203 6 -56 9 -61 30 -61 21 0 24 5 24
...
-16z"/>
            <path d="M4867 2724 c-4 -4 -7 -67 -7 -141 l0 -133 105 0 105 0 0 25 c0 24 -2
...
-98 -4z"/>
            <path d="M5280 2591 l0 -141 24 0 c20 0 24 6 30 40 6 40 6 40 52 40 86 0 137
...
53 -18z"/>
            <path d="M5580 2590 l0 -141 98 3 c89 3 97 5 97 23 0 18 -8 20 -67 23 l-68 3
-1 37 c0 20 -2 71 -2 112 -2 74 -2 75 -29 78 l-28 3 0 -141z"/>
            <path d="M5916 2708 c-14 -21 -116 -245 -116 -254 0 -2 13 -4 29 -4 22 0 31 6
...
-40z"/>
            <path d="M6110 2705 c0 -23 4 -25 40 -25 l39 0 3 -112 3 -113 28 -3 27 -3 0
115 0 115 43 3 c34 2 42 7 42 23 0 19 -8 20 -112 23 l-113 3 0 -26z"/>
            <path d="M6407 2723 c-4 -3 -7 -67 -7 -140 l0 -134 28 3 c25 3 27 7 30 50 l3
47 57 3 c50 3 57 6 60 25 3 20 -1 22 -55 25 l-58 3 -3 38 -3 37 70 0 c69 0 71
1 71 25 0 25 0 25 -93 25 -52 0 -97 -3 -100 -7z"/>
            <path d="M6730 2712 c-97 -49 -106 -186 -15 -242 68 -42 179 -16 212 50 17 34
...
-111 24 -17 25 -20 79 -4 108 18 35 81 49 121 28z"/>
            <path d="M7017 2709 c-3 -12 -3 -75 0 -140 6 -116 7 -119 30 -119 20 0 23 5
...
97 -14z"/>
            <path d="M7327 2724 c-4 -4 -7 -67 -7 -141 l0 -133 30 0 29 0 3 78 3 77 39
...
4 -28 42 -52 83 -42 70 -71 94 -91 74z"/>
        ...
        {/* ── Halos de nodos (blur difuso) ── */}
        <g filter={`url(#${uid}-soft)`}>
          <circle className={`zfr-nglow-${uid} g0`} cx="613" cy="405" r="19" />
          <circle className={`zfr-nglow-${uid} g1`} cx="699" cy="395" r="19" />
          <circle className={`zfr-nglow-${uid} g2`} cx="562" cy="462" r="19" />
          <circle className={`zfr-nglow-${uid} g3`} cx="646" cy="469" r="19" />
        </g>

        {/* ── Emblema ── */}
        <g className={animated ? `zfr-emblem-${uid}` : undefined}
          style={!animated ? { opacity: 1 } : undefined}
        >
          <use href={`#${uid}-embPaths`} fill={`url(#${uid}-gEmb)`} />
          {/* Destello sweep sobre la figura */}
          <g mask={`url(#${uid}-mEmb)`}>
            <rect
              className={animated ? `zfr-shine-${uid}` : undefined}
              x="230" y="230" width="240" height="380"
              fill={`url(#${uid}-gShine)`}
            />
          </g>
        </g>
        <mask id={`${uid}-mEmb`}>
          <use href={`#${uid}-embPaths`} fill="#fff" />
        </mask>
        ...
        {showText && (
          <use
            href={`#${uid}-wordPaths`}
            fill="#1de4f2"
            className={animated ? `zfr-wordmark-${uid}` : undefined}
        ...
        {showText && showSubtitle && (
          <use
            href={`#${uid}-subPaths`}
            fill="#1db5e2"
            className={animated ? `zfr-subtitle-${uid}` : undefined}
            style={!animated ? { opacity: 1 } : undefined}
          />
        )}
      </svg>
...
/**
 * ZefirusIcon — Solo el emblema sin texto, para favicons y espacios compactos.
 */
export function ZefirusIcon({ size = 24 }: { size?: number }) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`${uid}-g`} x1="0" y1="0" x2="100" y2="300" gradientUnits="userSpaceOnUse">
          <stop offset="0"   stopColor="#28eefb" />
          <stop offset=".45" stopColor="#23d9ea" />
          <stop offset=".8"  stopColor="#17a5ec" />
          <stop offset="1"   stopColor="#1490f0" />
        </linearGradient>
        <g id={`${uid}-ep`} transform="translate(0,820) scale(0.1,-0.1)">
          <path d="M6850 7659 c-223 -59 -451 -120 -507 -135 -57 -14 -103 -30 -103 -35
0 -5 54 -56 120 -115 67 -59 119 -111 118 -115 -9 -25 -463 -524 -623 -685
-270 -271 -465 -412 -751 -544 -119 -55 -252 -101 -649 -225 -82 -26 -188 -62
-234 -81 -47 -19 -87 -32 -89 -29 -5 4 35 50 273 315 198 220 477 534 638 717
...
120 76 225 116 85 33 110 39 110 28z"/>
      ...
      <use href={`#${uid}-ep`} fill={`url(#${uid}-g)`} />
    </svg>
  );
}

...
export function getFatigueDisplay(level: FatigueAnalysis["level"]) {
  switch (level) {
    ...
    case "healthy":
      return { label: "Estable", color: "var(--cyan)", bg: "rgba(0,212,255,0.06)", border: "rgba(0,212,255,0.15)", icon: "🔵" };
    case "warning":
      return { label: "Fatigando", color: "var(--amber)", bg: "rgba(251,191,36,0.08)", border: "rgba(251,191,36,0.2)", icon: "🟡" };
    case "critical":
      return { label: "Fatigado", color: "var(--red)", bg: "rgba(239,68,68,0.08)", border: "rgba(239,68,68,0.2)", icon: "🔴" };
  }
}

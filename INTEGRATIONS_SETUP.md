...
Guía para configurar cada plataforma publicitaria en FlowChart. El framework OAuth genérico (`/api/oauth/[provider]/start` → `/callback`) maneja la conexión; solo necesitas crear la app de desarrollador y agregar las variables en Vercel.
...
5. Conectar desde el Dashboard de FlowChart → Integraciones → WhatsApp Business, o via API:
   ```bash

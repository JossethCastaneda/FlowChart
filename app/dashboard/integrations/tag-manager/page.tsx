  ...
  return (
    <>
      <style>{`
        ...
        .adaccount-row { display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: var(--fc-surface-hover); border: 1px solid var(--fc-border); border-radius: 8px; margin-bottom: 8px; transition: border-color 0.2s; }
              ...
              {accounts.map(account => (
                <div key={account.accountId} className="adaccount-row">
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ margin: 0, fontSize: 11, color: "var(--fc-text-secondary)" }}>ID: {account.accountId}</p>
                  </div>
                  <button
                    onClick={() => fetchContainers(account.accountId)}
                    style={{
                      padding: "6px 12px", borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: "pointer",
                      background: "var(--fc-surface-hover)", border: "1px solid var(--fc-border)", color: "var(--fc-text)",
                    }}
                  >
                    {t.viewContainers}
                  </button>
            ...
            <div style={{ animation: "fadeIn 0.2s ease" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
                <button
                  ...
                  style={{ background: "none", border: "none", padding: 0, color: "var(--fc-text-secondary)", fontSize: 13, cursor: "pointer", textDecoration: "underline" }}
                >
                  {t.backToAccounts}
                </button>
                <span style={{ color: "var(--fc-text-muted)" }}>/</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: "var(--fc-text)" }}>{accounts.find(a => a.accountId === selectedAccountId)?.name}</span>
              </div>
              
              <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--fc-text)", margin: "0 0 16px" }}>{t.enabledContainers}</h2>
              ...
              {loadingContainers ? (
                ...
                <div style={{ padding: "40px 20px", textAlign: "center", borderRadius: 12, border: "1px dashed var(--fc-border)", background: "var(--fc-surface)" }}>
                  <p style={{ color: "var(--fc-text-secondary)", fontSize: 13, margin: 0 }}>{t.noContainers}</p>
                </div>
              ) : (
                containers.map(container => {
                  ...
                  return (
                    <div key={container.containerId} className="adaccount-row clickable" data-selected={isSelected} onClick={() => toggleSelection(selectedAccountId, container.containerId)}>
                      <div style={{ width: 32, height: 32, borderRadius: "50%", background: "var(--fc-success)", opacity: 0.2, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--fc-success)", flexShrink: 0, position: "relative" }}>
                      </div>
                      <div style={{ position: "absolute", marginLeft: 8, color: "var(--fc-success)" }}>
                        <Box size={16} />
                      </div>
                      <div style={{ flex: 1, marginLeft: 20 }}>
                        <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{container.name}</p>
                        <p style={{ margin: 0, fontSize: 11, color: "var(--fc-text-secondary)" }}>{container.publicId} (ID: {container.containerId})</p>
                      </div>
                      <div style={{

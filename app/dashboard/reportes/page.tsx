...
export default function ReportesPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }} className="page-enter">
      ...
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", paddingBottom: 16, borderBottom: "1px solid var(--fc-border-subtle)" }}>
        <Link href="/dashboard/proyectos" style={{
          display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 18px",
          background: "var(--fc-surface)", border: "1px solid rgba(224,96,126,0.25)",
      ...
      {loading && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 0", gap: 12 }}>
          <Loader2 style={{ width: 18, height: 18, color: "var(--fc-accent)", animation: "spin 1s linear infinite" }} />
          <p style={{ fontSize: 12, color: "var(--fc-text-muted)" }}>Cargando reportes...</p>
        </div>
      )}
      ...
      {!loading && reports.length === 0 && (
        <div style={{
          ...
          padding: "52px 24px", gap: 16, background: "var(--fc-surface)", border: "1px solid var(--fc-border)",
          borderRadius: 16, textAlign: "center",
        }}>
          <div style={{
            width: 56, height: 56, borderRadius: 16, background: "var(--fc-surface)",
          ...
          <div>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: "var(--fc-text)", margin: "0 0 8px" }}>
      ...
      {!loading && reports.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 14 }}>
          {reports.map((r) => (
            <div key={r.id} style={{
              background: "var(--fc-surface)", border: "1px solid var(--fc-border)", borderRadius: 14,
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--fc-text)", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {r.title}
                  </h3>
                  <p style={{ fontSize: 11, color: "var(--fc-text-muted)", margin: "4px 0 0" }}>
                ...
                <div style={{
                  ...
                  color: r.status === "active" ? "var(--fc-success)" : "var(--fc-text-muted)",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <Calendar style={{ width: 12, height: 12, color: "var(--fc-text-muted)" }} />
              ...
              {/* Actions */}
              <div style={{ display: "flex", gap: 8, marginTop: "auto", paddingTop: 8, borderTop: "1px solid var(--fc-border-subtle)" }}>
                <Link href={`/reportes/${r.slug}`} target="_blank" style={{
                  display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 12px",
                  background: "var(--fc-surface-hover)", border: "1px solid var(--fc-border)",
                  color: "var(--fc-text)", borderRadius: 8, fontWeight: 600, fontSize: 11,
                ...
                <button onClick={() => copyPublicLink(r.slug)} style={{
                  ...
                  {copiedSlug === r.slug
                    ? <><Check style={{ width: 12, height: 12, color: "var(--fc-success)" }} /> Copiado</>
                ...
                <button onClick={() => handleDelete(r.id)} style={{
                  ...
                  color: "var(--fc-danger)", borderRadius: 8, fontWeight: 600, fontSize: 11,

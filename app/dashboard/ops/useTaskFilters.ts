...
export function useTaskFilters() {
  ...
  useEffect(() => {
    try {
      const r = localStorage.getItem("FlowChart Core:ops-prefs");
      if (r) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("FlowChart Core:ops-prefs", JSON.stringify({ groupBy, viewMode, fAssignee, fPriority, fTag, fArea }));
    } catch { /* ignore */ }

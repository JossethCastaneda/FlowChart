...
import { CreateAdModal } from "@/components/ads-manager/CreateAdModal";
...
const ALL_COLUMNS = [
  ...
  { key: "cost_per_atc", label: "Costo por ATC" },
  { key: "initiate_checkout", label: "Inicio de pago" },
  { key: "cost_per_ic", label: "Costo por inicio de pago" },
  // Clics avanzados
  { key: "outbound_clicks", label: "Clics salientes" },
  { key: "outbound_ctr", label: "CTR saliente" },
  { key: "unique_ctr", label: "CTR único" },
  // Video avanzado
  { key: "thruplay", label: "ThruPlays" },
  { key: "thruplay_rate", label: "ThruPlay Rate %" },
  { key: "cost_per_thruplay", label: "Costo por ThruPlay" },
    ...
    if (platform === "google") {
      ...
      try {
        ...
        if (failCount === 0) {
          ...
          addAlert({ severity: "success", title: "Actualización en Google Ads", message: `${successCount} ${levelLabel}(s) cambiaron de estado exitosamente.`, source: "gasto" });
        } else {
          addToast("warning", ` ${successCount} actualizadas. ${failCount} fallaron.`);
          addAlert({ severity: "warning", title: "Actualización Parcial (Google Ads)", message: `${successCount} actualizadas, ${failCount} fallidas.`, source: "gasto" });
        }
        fetchData();
        setSelectedIds([]);
            } catch (err: any) {
      ...
      if (data.success) {
        ...
        if (failCount === 0) {
          ...
          if (archivedCount > 0) {
            ...
            addAlert({ severity: "success", title: "Acción completada", message: `${successCount} ${levelLabel}(s) procesadas correctamente (${action}).`, source: "gasto" });
          }
        } else {
          addToast("warning", `${successCount} de ${n} ${action} exitosas. ${failCount} fallaron.`);
          addAlert({ severity: "warning", title: "Acción Parcial en Meta", message: `${failCount} elementos fallaron durante la acción ${action}.`, source: "gasto" });
        }
        fetchData();
        setSelectedIds([]);
      } else {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" style={{
      ...
      >
        ...
        {/* Create ad — needs at least one adset loaded */}
        {platform !== "google" && !loadingAccounts && selectedAccountId && selectedAccountId !== "all" && adsets.length > 0 && (
          <button
      ...
      {showCreateAd && selectedAccountId && selectedAccountId !== "all" && (
        <CreateAdModal
          ...
          onCreated={() => {
            ...
            addToast("success", "✅ Anuncio creado en pausa");
            setActiveLevel("ads");
            fetchData();
          }}
        />

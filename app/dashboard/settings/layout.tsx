    ...
    <div className="flex flex-col h-[calc(100vh-88px)] overflow-hidden">
      <div className="shrink-0 pb-3">
        <PageHeader
          ...
          icon={<Settings className="w-6 h-6 text-[var(--cyan)]" />}
        />
      </div>
        ...
        >
          ...
          <div className="relative mb-3 shrink-0">
            ...
            <input
              ...
              className="w-full bg-[var(--surface-hover)] border border-white/5 text-[var(--foreground)] text-xs rounded-lg pl-9 pr-8 py-2 outline-none transition-all focus:border-blue-500 focus:ring-1 focus:ring-blue-500/40"
            />
            {query && (
              <button
                onClick={() => setQuery("")}
                aria-label="Limpiar búsqueda"
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[var(--text-muted)] hover:text-[var(--foreground)]"
              >
          ...
          {visibleGroups.length === 0 && (
            <p className="text-[11px] text-[var(--text-muted)] px-3 py-4">
            ...
            {visibleGroups.map((group, groupIndex) => (
              <div key={group.group} className="flex flex-row md:flex-col gap-1 md:gap-0 shrink-0">
                <div className="hidden md:block">
                  ...
                  <div className="text-[9px] font-extrabold tracking-widest uppercase text-[var(--text-muted)] px-3 pb-2">
                    {group.group}
                  </div>
                </div>
                        ...
                        <span className="relative z-10 flex items-center gap-2 md:gap-3 w-full">
                          <Icon
                            className={`w-4 h-4 md:w-[15px] md:h-[15px] shrink-0 ${
                              active
                                ? "text-[var(--cyan)] drop-shadow-[0_0_8px_rgba(34,211,238,0.5)]"
                                : "text-[var(--text-muted)]"
                            }`}
                          />
                          ...
                          {active && (
                            <ChevronRight className="hidden md:block w-3 h-3 text-[var(--cyan)] opacity-60 ml-auto" />
                          )}
                        </span>
                      </Link>
                    );

...
export default function LoginPage() {
  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#0a0618", fontFamily: "var(--font-inter),system-ui,sans-serif", overflow: "hidden", padding: "24px" }}>
      <style>{`
        ...
        @media (max-width:480px){
          .login-card { padding: 32px 24px; border-radius: 16px; background: rgba(10,10,12,0.8); border: 1px solid rgba(255,255,255,0.05); }
        }
      ...
      <div className="login-card">
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <h1 style={{ fontFamily: "var(--font-inter),system-ui,sans-serif", fontSize: "24px", fontWeight: 700, margin: 0, color: "#fff", letterSpacing: "-0.02em" }}>
            {t.title}
          </h1>
        </div>
        ...
        {success ? (
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            <div style={{ width: 64, height: 64, margin: "0 auto 24px", borderRadius: "50%", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginTop: 8 }}>{t.successSub}</div>
          </div>
        ) : showReturning ? (
          <>
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              <div style={{ width: 64, height: 64, margin: "0 auto 16px", borderRadius: "50%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 600, color: "#fff" }}>{(savedName || savedEmail || "U").trim().charAt(0).toUpperCase()}</div>
              <div style={{ fontSize: 20, fontWeight: 600, color: "#fff", letterSpacing: "-0.01em" }}>{t.returningHi(savedName || "")}</div>
              <div style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginTop: 4 }}>{savedEmail}</div>
            </div>
            <div style={{ position: "relative", marginBottom: 16 }}>
              <input type={showPw ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setCredError(""); }} placeholder={t.pwReturning} onFocus={onFocus} onBlur={onBlur} style={{ ...input, marginBottom: 0, paddingRight: 44, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }} />
              {EyeBtn}
            </div>
            {ErrorRow}
            <button onClick={submit} disabled={isLoading} style={{ ...cta, opacity: isLoading ? 0.7 : 1, background: "#fff", color: "#000" }} onMouseEnter={(e) => e.currentTarget.style.background = "#e0e0e0"} onMouseLeave={(e) => e.currentTarget.style.background = "#fff"}>
            ...
            <div style={{ textAlign: "center", marginTop: 24 }}>
              <a href="#" onClick={(e) => { e.preventDefault(); setUseOther(true); setPassword(""); setCredError(""); }} style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", textDecoration: "none" }}>{t.useOther}</a>
            </div>
          </>
        ) : (
          <>
            <div style={{ display: "flex", gap: 12, marginBottom: 32 }}>
              <button onClick={handleFacebookLogin} disabled={isLoading || !providerStatusLoaded || !hasFacebookProvider} style={{ flex: 1, padding: 12, border: "1px solid rgba(255,255,255,0.2)", cursor: "pointer", background: "rgba(255,255,255,0.03)", color: "#fff", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 12, transition: "all 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.1)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.3)"; }} onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.03)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.2)"; }}>
              ...
              <button onClick={handleGoogleLogin} disabled={isLoading || !providerStatusLoaded || !hasGoogleProvider} style={{ flex: 1, padding: 12, border: "1px solid rgba(255,255,255,0.2)", cursor: "pointer", background: "rgba(255,255,255,0.03)", color: "#fff", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 12, transition: "all 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.1)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.3)"; }} onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.03)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.2)"; }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32 }}>
              <span style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.1)" }} /><span style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>{t.orEmail}</span><span style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.1)" }} />
            </div>

            <div style={{ display: "flex", gap: 4, marginBottom: 24, padding: 4, background: "rgba(255,255,255,0.05)", borderRadius: 12 }}>
            ...
            {isRegister && <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t.name} onFocus={onFocus} onBlur={onBlur} style={{ ...input, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }} />}
            <input type="email" value={email} onChange={(e) => { setEmail(e.target.value); setCredError(""); }} placeholder="tu@empresa.com" onFocus={onFocus} onBlur={onBlur} style={{ ...input, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }} />
            <div style={{ position: "relative", marginBottom: isRegister ? 16 : 8 }}>
              <input type={showPw ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setCredError(""); }} placeholder="••••••••" onFocus={onFocus} onBlur={onBlur} style={{ ...input, marginBottom: 0, paddingRight: 44, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }} />
              {EyeBtn}
            </div>
            {isRegister && <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t.confirmPw} onFocus={onFocus} onBlur={onBlur} style={{ ...input, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }} />}
            
            {ErrorRow}
            {!isRegister && <div style={{ textAlign: "right", marginBottom: 28 }}><a href="/forgot-password" style={{ fontSize: 13, color: "#00d4ff", textDecoration: "none", transition: "color 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.color = "#fff"} onMouseLeave={(e) => e.currentTarget.style.color = "#00d4ff"}>{t.forgot}</a></div>}

            <button onClick={submit} disabled={isLoading} style={{ ...cta, opacity: isLoading ? 0.7 : 1, marginTop: isRegister ? 24 : 0, background: "#fff", color: "#000" }} onMouseEnter={(e) => e.currentTarget.style.background = "#e0e0e0"} onMouseLeave={(e) => e.currentTarget.style.background = "#fff"}>

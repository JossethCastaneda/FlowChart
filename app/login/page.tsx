...
export default function LoginPage() {
  ...
  const input: React.CSSProperties = {
    width: "100%", padding: "14px 16px", marginBottom: 16, background: "rgba(0,0,0,0.4)",
  ...
  const cta: React.CSSProperties = {
    ...
    justifyContent: "center", gap: 10, minHeight: 52, fontSize: 14,
  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#060606", fontFamily: "var(--font-inter),system-ui,sans-serif", overflow: "hidden" }}>
      <style>{`
        ...
        .login-card {
          width: 100%; maxWidth: 460px; padding: 48px; 
          background: rgba(10,12,18,0.7); 
          border: 1px solid rgba(0, 212, 255, 0.12); 
          border-radius: 24px; 
          box-shadow: 0 30px 80px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.05), inset 0 0 40px rgba(0,212,255,0.05);
        ...
        .col-funnel-container {
          position: absolute; top: 0; left: 0; right: 0; bottom: 0; pointer-events: none; z-index: 0; opacity: 0.8;
        }
      ...
      {/* Language toggle */}
      <div style={{ position: "absolute", top: 24, right: 24, zIndex: 20, display: "flex", gap: 2, padding: 4, background: "rgba(10,12,18,0.7)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 100, backdropFilter: "blur(10px)" }}>
      ...
      <div style={{ position: "relative", zIndex: 10, textAlign: "center", marginBottom: 32, display: "flex", flexDirection: "column", alignItems: "center" }}>
        <a href="/" style={{ display: "inline-flex", marginBottom: 28 }}><SodareLogo size="lg" /></a>
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", marginBottom: 12 }}>
          <Orbi size={48} style={{ filter: "drop-shadow(0px 0px 15px rgba(0, 212, 255, 0.5))" }} />
        </div>
        <h1 className="text-shimmer login-headline" style={{ fontFamily: ORB, fontSize: "clamp(26px, 4vw, 36px)", fontWeight: 700, lineHeight: 1.15, margin: 0, padding: "0 20px" }}>
          {t.h1}
        </h1>
      </div>
      ...
      <div className="login-card">
        {success ? (
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            <div style={{ width: 64, height: 64, margin: "0 auto 24px", borderRadius: "50%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--emerald)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
            </div>
          ...
          <>
            ...
            <button onClick={submit} disabled={isLoading} style={{ ...cta, opacity: isLoading ? 0.7 : 1 }} onMouseEnter={(e) => e.currentTarget.style.transform = "translateY(-2px)"} onMouseLeave={(e) => e.currentTarget.style.transform = "translateY(0)"}>
              {isLoading ? <span style={{ width: 18, height: 18, border: "2px solid rgba(0,0,0,0.2)", borderTopColor: "#fff", borderRadius: "50%", animation: "f-spin .7s linear infinite", display: "inline-block" }} /> : t.ctaLogin}
            </button>
          ...
          <>
            <div style={{ display: "flex", gap: 12, marginBottom: 32 }}>
              <button onClick={handleFacebookLogin} disabled={isLoading || !providerStatusLoaded || !hasFacebookProvider} style={{ flex: 1, padding: 12, border: "1px solid rgba(255,255,255,0.08)", cursor: "pointer", background: "rgba(255,255,255,0.03)", color: "#fff", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 8, transition: "background 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.08)"} onMouseLeave={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.03)"}>
              ...
              <button onClick={handleGoogleLogin} disabled={isLoading || !providerStatusLoaded || !hasGoogleProvider} style={{ flex: 1, padding: 12, border: "1px solid rgba(255,255,255,0.08)", cursor: "pointer", background: "rgba(255,255,255,0.03)", color: "#fff", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 8, transition: "background 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.08)"} onMouseLeave={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.03)"}>
            ...
            <button onClick={submit} disabled={isLoading} style={{ ...cta, opacity: isLoading ? 0.7 : 1, marginTop: isRegister ? 24 : 0 }} onMouseEnter={(e) => e.currentTarget.style.transform = "translateY(-2px)"} onMouseLeave={(e) => e.currentTarget.style.transform = "translateY(0)"}>
              {isLoading ? <span style={{ width: 18, height: 18, border: "2px solid rgba(0,0,0,0.2)", borderTopColor: "#fff", borderRadius: "50%", animation: "f-spin .7s linear infinite", display: "inline-block" }} /> : (isRegister ? t.ctaReg : t.ctaLogin)}
            </button>
          </>
        )}
        ...
        {!success && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 40 }}>
            ...
            <span style={{ fontSize: 12, color: "var(--emerald)", fontWeight: 500 }}>{t.secure}</span>
          </div>
        )}
      </div>
    </div>

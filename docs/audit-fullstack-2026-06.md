...
**committer concurrente** (`FlowChart Dev <jtrejo.lid.mkt@gmail.com>`, otra sesión/proceso — ver
...
1. **`prisma db push` en cada build** (`build` script → `scripts/db-sync.mjs`). Es aditivo y no-fatal, PERO
   ...
   sin resolver** (CLAUDE.md: producción Vercel apunta a `ep-long-unit…c-7`, no a `flowchart-prod`/c-8), un build

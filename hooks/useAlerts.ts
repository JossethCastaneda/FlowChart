...
export function useAlerts(data: any[], level: "campaigns" | "adsets" | "ads"): Alert[] {
  return useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      // ── CRITICAL ──
      // Active with $0 spend
      if (item.status === "ACTIVE" && spend === 0 && item.effective_status === "ACTIVE") {
        alerts.push({
          id: `${item.id}-no-spend`,
          level: "critical",
          title: "Campaña activa sin gasto",
          message: `"${item.name}" está activa pero no está gastando. Verifica la configuración.`,
          itemId: item.id,
          itemName: item.name,
        });
      }

      // ROAS below 1 with significant spend
      if (roas > 0 && roas < 1 && spend > 20) {
        alerts.push({
          id: `${item.id}-low-roas`,
          level: "critical",
          title: "ROAS por debajo del punto de equilibrio",
          message: `"${item.name}" tiene ROAS de ${roas.toFixed(2)}x con $${spend.toFixed(0)} gastados. Estás perdiendo dinero.`,
          itemId: item.id,
          itemName: item.name,
        });
      }
      ...
      // Low CTR
      if (ctr < 0.5 && impressions > 5000 && item.status === "ACTIVE") {
        alerts.push({
          id: `${item.id}-low-ctr`,
          level: "warning",
          title: "CTR muy bajo",
          message: `"${item.name}" tiene CTR de ${ctr.toFixed(2)}%. Los creativos no están generando clics.`,
          itemId: item.id,
          itemName: item.name,
        });
      }
